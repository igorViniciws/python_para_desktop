...
def formatar_telefone(self, event):
    telefone = self.telefone_entry.get().replace("(", "").replace(")", "").replace("-", "").replace(" ", "")
    formatado = telefone
    if len(telefone) > 8:
        formatado = f"{telefone[:2]} {telefone[2:7]}-{telefone[7:]}"
    elif len(telefone) > 2:
        formatado = f"({telefone[:2]}) {telefone[2:]}"
...
class Telalista(ctk.CTkToplevel):
    ...
    def setup_ui(self):
        ...
        # Frame para os botões
        self.btn_frame = ctk.CTkFrame(self.frame, fg_color="transparent")
        self.btn_frame.pack(pady=12, padx=18)
        ...
        # Configura o estilo de Treeview de acordo com o tema
def configurar_estilo_treeview(self):
    ...
    if modo == "Dark":
        ...
        self.style.configure("Treeview.Heading",
                           ...
                           foreground="white",
                           font=("Roboto", 14))
        self.style.map('Treeview', background=[('selected', '#225596')])
        ...
        self.style.map("Treeview.Heading", background=[('active', '#2345a7')])
...
def formatar_telefone_update(self, event, entry):
    ...
    if len(telefone) > 2:
        formatado = f"({telefone[:2]}) ({telefone[2:7]})";
    if len(telefone) > 7:
        formatado = f"({telefone[:2]}) ({telefone[2:7]})-{telefone[7:]}";
